...
export default function ReportTreatmentCard({
 ...
 return (
 <div
 className="animate-result-card bg-white border border-gray-200 rounded-xl overflow-hidden"
 style={{ animationDelay: `${delay}ms` }}
 >
 {/* Collapsed header — always visible */}
 <button
 onClick={() => setExpanded(!expanded)}
 className="w-full text-left p-4 md:p-5 flex items-start gap-4 hover:bg-gray-50/50 transition-colors"
 >
 {/* Rank badge */}
 ...
 <h3 className="font-serif text-base md:text-lg text-gray-900 truncate">
 {t.name}
 </h3>
 ...
 {/* Mechanism snippet */}
 {result.mechanism && (
 <p className="text-sm text-gray-600 font-sans line-clamp-2 leading-relaxed">
 {result.mechanism}
 </p>
 )}

 {/* Quick stats row */}
 <div className="flex flex-wrap items-center gap-3 mt-2.5 text-xs text-gray-500 font-sans">
 {result.topOutcome && (
 ...
 {/* Expanded details */}
 {expanded && (
 <div className="border-t border-gray-100 px-4 md:px-5 pb-5 pt-4 space-y-5 animate-blueprint-fade-in">
 {/* Evidence & Study */}
 {result.topOutcome && (
 <div>
 <h4 className="text-xs font-sans font-semibold text-gray-400 uppercase tracking-wider mb-2">
 Top Evidence
 </h4>
 ...
 <div className="bg-gray-50 rounded-lg p-3.5">
 <p className="text-sm font-sans font-medium text-gray-800 leading-snug">
 ...
 <div className="flex items-start gap-3 bg-terracotta/[0.04] rounded-lg p-3.5">
 <div className="flex-shrink-0 w-8 h-8 rounded-full bg-terracotta/10 flex items-center justify-center text-sm">
 {t.icon}
 </div>
 <div>
 ...
 <blockquote className="mt-2.5 pl-3 border-l-2 border-terracotta/30">
 <p className="text-sm text-gray-600 font-sans italic leading-relaxed">
 &ldquo;{result.communityData.quotes[0].text}&rdquo;
 </p>
 <cite className="text-xs text-gray-400 font-sans not-italic">
 ...
 <span
 key={effect}
 className="px-2 py-0.5 bg-amber-50 text-xs text-amber-700 rounded-full font-sans"
 >
 {effect}
 </span>
 ))}
 ...
 className="inline-flex items-center gap-1.5 text-sm font-sans text-terracotta hover:underline"
 >
