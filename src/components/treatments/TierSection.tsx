...
export default function TierSection({ tier, treatments, defaultOpen = false }: TierSectionProps) {
  ...
  return (
    <div className={`border rounded-2xl overflow-hidden transition-colors ${tier.borderClass} ${open ? tier.bgClass : "bg-white dark:bg-[#1C1815]"}`}>
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2.5">
            ...
            <span className="text-xs text-gray-400 dark:text-[#6B6358] font-sans">
              {tier.scoreMin}–{tier.scoreMax}
            </span>
          </div>
          <p className="text-xs text-gray-500 dark:text-[#A89F90] font-sans mt-0.5 hidden sm:block">
            {tier.description}
          </p>
        </div>
