...
export default function TherapyExplorer(){
 ...
 const filtered=catalogue.filter(t=>(category==='All'||t.category===category)&&`${t.name} ${t.title} ${t.category}`.toLowerCase().includes(query.toLowerCase()));
 ...
 useEffect(()=>{if(!atlas)return;const focused=focusedRegion!=null?therapy.regions[focusedRegion]:null;const ids=focused?focused.elements:allMappedIds;setScene(s=>({...s,selected:ids??[],isolate:!!focused,explode:0,rotate:false,reset:s.reset+1}));setInspected(null);},[atlas,therapy,focusedRegion,allMappedIds]);
 ...
 const choose=(id:string,keepCombination=false)=>{setSelectedId(id);setFocusedRegion(null);setPlaying(false);setInspected(null);setPanelOpen(true);setMobileLibrary(false);if(!keepCombination)setSelectedCombination(null);history.replaceState(null,'',`${location.pathname}?t=${id}`);};
 ...
 return <div className={styles.explorer} data-lenis-prevent>
  ...
  {panelOpen&&<aside className={`${styles.detail} detail-sheet`} aria-label={`${therapy.name} details`}>
   <div className={styles.detailHeading}><div><span className={styles.eyebrow}>{inspected?'ANATOMY':therapy.category}</span><h1>{inspected?inspected.name:therapy.name}</h1></div><button onClick={()=>{setPanelOpen(false);setPlaying(false);}} aria-label="Close therapy details"><X size={20}/></button></div>
   ...
   {!inspected&&<div className={styles.detailFoot}><span>{therapy.kind}</span><span>{therapy.regions.length} mapped regions</span></div>}
  </aside>}
  ...
  <div className={styles.sceneCaption}>{scene.isolate?(inspected?.name??(focusedRegion!=null?therapy.regions[focusedRegion]?.name:therapy.name)):activeCombination?.name??therapy.name}<span>{scene.isolate?'ANATOMICAL FOCUS':'DRAG TO ROTATE · SCROLL TO ZOOM'}</span></div>
