...
// Evidence-tier colors (matches the Kamura Score tiers used across the site).
const tierRGB=(score:number|null|undefined):[number,number,number]=>score==null?[154,139,124]:score>=85?[196,168,130]:score>=70?[22,163,74]:score>=50?[37,99,235]:[234,88,12];
const tierHex=(score:number|null|undefined)=>{const [r,g,b]=tierRGB(score);return `rgb(${r},${g},${b})`;};
export default function TherapyExplorer(){
 ...
 const [goal,setGoal]=useState<string|null>(null),[goalOverview,setGoalOverview]=useState(false),[welcome,setWelcome]=useState(false);
 const therapy=catalogue.find(t=>t.id===selectedId)!;
 const goalRanked=useMemo(()=>goal?catalogue.filter(t=>t.goals.includes(goal)).sort((a,b)=>(b.kamuraScore??-1)-(a.kamuraScore??-1)):[],[goal]);
 ...
 // Goal overview: every relevant therapy's targets, tinted by evidence tier — the heatmap.
 const goalScene=useMemo(()=>{if(!goalOverview||!goal)return null;const ids:string[]=[],colors:Record<string,[number,number,number]>={};goalRanked.forEach(t=>{const c=tierRGB(t.kamuraScore);t.regions.forEach(r=>r.elements.forEach(el=>{if(!(el in colors)){colors[el]=c;ids.push(el);}}));});return{ids,colors};},[goalOverview,goal,goalRanked]);
 ...
 useEffect(()=>{try{if(!localStorage.getItem('kamura.atlas.welcomed')&&!new URLSearchParams(location.search).get('t'))setWelcome(true);}catch{}},[]);
 ...
 const dismissWelcome=(goalId?:string)=>{setWelcome(false);try{localStorage.setItem('kamura.atlas.welcomed','1');}catch{}if(goalId)selectGoal(goalId);};
 ...
 const reset=()=>{setPlaying(false);setFocusedRegion(null);setInspected(null);setScene(s=>({...initial,selected:allMappedIds,reset:s.reset+1}));};
 return <div className={styles.explorer} data-lenis-prevent>
  ...
  <header className={styles.header}>
   ...
   <div className={styles.headerActions}><button onClick={()=>setMobileLibrary(v=>!v)} className={styles.libraryToggle}><Search size={17}/>Therapies</button><button onClick={()=>setCredits(true)} aria-label="Sources and model information"><CircleHelp size={20}/></button></div>
  </header>
  <aside className={`${styles.library} ${mobileLibrary?styles.libraryOpen:''}`} aria-label="Therapy library">
   ...
   {library==='therapies'?<><div className={styles.goalRow}>{GOALS.map(g=><button key={g.id} className={styles.goalChip} aria-pressed={goal===g.id} onClick={()=>selectGoal(goal===g.id?null:g.id)}>{g.label}</button>)}</div><label className={styles.search}><Search size={16}/><input aria-label="Search therapies" placeholder="Peptide, hormone, pathway…" value={query} onChange={e=>setQuery(e.target.value)}/>{query&&<button onClick={()=>setQuery('')} aria-label="Clear therapy search"><X size={15}/></button>}</label><label className={styles.category}><span className={styles.srOnly}>Category</span><select value={category} onChange={e=>setCategory(e.target.value)}>{categories.map(c=><option key={c}>{c}</option>)}</select><ChevronDown size={14}/></label><div className={styles.therapyList}>{filtered.map(t=><button className={styles.therapyRow} key={t.id} aria-pressed={t.id===selectedId&&!activeCombination} onClick={()=>choose(t.id)}><span className={styles.therapyDot}/><span><strong>{t.name}</strong><small>{t.kind}</small></span>{t.kamuraScore?<span className={styles.rowScore}>{t.kamuraScore}</span>:<ArrowRight size={14}/>}</button>)}{filtered.length===0&&<p className={styles.empty}>No therapies match this search.</p>}</div></>:<div className={styles.therapyList}><p className={styles.listIntro}>Explore constituent pathways together.</p>{combinations.map(c=><button key={c.name} className={styles.comboRow} aria-pressed={activeCombination?.name===c.name} onClick={()=>{setSelectedCombination(c.name);choose(c.ids[0],true);}}><strong>{c.name}</strong><span>{c.ids.map(id=>catalogue.find(t=>t.id===id)?.name).join(' · ')}</span><small>{c.ids.length} therapies <ArrowRight size={13}/></small></button>)}</div>}
  ...
  {panelOpen&&<aside className={`${styles.detail} detail-sheet`} aria-label={`${therapy.name} details`}>
   ...
   <div className={styles.detailScroll}>
    {goalOverview?<>
    <p className={styles.lead}>Every therapy we&rsquo;ve mapped for this goal, ranked by evidence — strongest first. The body shows where they act, tinted by evidence tier.</p>
    <section className={styles.regionList}>
     {goalRanked.map((t,i)=><button key={t.id} onClick={()=>choose(t.id)}><span>{String(i+1).padStart(2,'0')}</span><i className={styles.gradeDot} style={{background:tierHex(t.kamuraScore)}}/><strong>{t.name}</strong>{t.kamuraScore?<small>{t.kamuraScore}</small>:<small>{t.researchContext}</small>}<ArrowRight size={14}/></button>)}
    </section>
    <p className={styles.retrieved}>Gold 85+ · Green 70–84 · Blue 50–69 · Grey = not yet scored. Educational only, not medical advice.</p>
    ...
    {goal&&<div className={styles.goalHead}><strong>{GOALS.find(g=>g.id===goal)?.label}</strong><button onClick={()=>selectGoal(goal)}>&larr; All ranked for this goal</button></div>}
    <div className={styles.badgeRow}>
     <span className={styles.researchBadge}>{therapy.researchContext}</span>
     {therapy.kamuraScore&&<Link className={styles.scoreChip} href={`/treatments/${therapy.treatmentSlug}`}>Kamura Score <strong>{therapy.kamuraScore}</strong><span>· {therapy.evidenceLevel} evidence</span><ArrowUpRight size={12}/></Link>}
     {therapy.uaeAvailable&&<span className={styles.uaeChip}>UAE · {therapy.costEstimate??'available'}</span>}
    </div>
    <p className={styles.verdict}>&ldquo;{therapy.verdict}&rdquo;</p>
    ...
    {/* 2 · Where it acts */}
    <section className={styles.regionList}>
     <div className={styles.sectionLabel}><MapPin size={14}/>Where it acts</div>
    ...
    <section className={styles.researchList}>
     ...
     {therapy.treatmentSlug&&<Link className={styles.researchLink} href={`/treatments/${therapy.treatmentSlug}`}><ArrowUpRight size={17}/><span>Kamura Score &amp; full evidence breakdown</span><ArrowRight size={16}/></Link>}
     ...
     <p className={styles.retrieved}>References retrieved {therapy.retrieved} · Educational only, not medical advice. Several entries are experimental compounds not approved for human use.</p>
    </section>
    </>}
   </div>
  ...
  <div className={styles.sceneCaption}>{goalOverview?GOALS.find(g=>g.id===goal)?.label:scene.isolate?(inspected?.name??(focusedRegion!=null?therapy.regions[focusedRegion]?.name:therapy.name)):activeCombination?.name??therapy.name}<span>{goalOverview?'EVIDENCE MAP · TINTED BY TIER':scene.isolate?'ANATOMICAL FOCUS':'DRAG TO ROTATE · SCROLL TO ZOOM'}</span></div>
  {welcome&&<div className={styles.welcome} role="dialog" aria-label="Welcome to Kamura">
   <div className={styles.welcomeCard}>
    <span>KAMURA · PREVENTION &amp; LONGEVITY</span>
    <h2>What actually works — <em>mapped to your body</em>.</h2>
    <p>Kamura grades longevity therapies on real evidence. Pick a goal to see where each therapy acts, how it works, what it costs in the UAE — and whether it&rsquo;s worth it.</p>
    <div className={styles.welcomeGoals}>{GOALS.map(g=><button key={g.id} className={styles.goalChip} onClick={()=>dismissWelcome(g.id)}>{g.label}</button>)}</div>
    <button className={styles.welcomeStart} onClick={()=>dismissWelcome()}>Just let me explore <ArrowRight size={15}/></button>
    <p className={styles.welcomeFoot}>2,234 anatomical structures · 28 therapies · every claim linked to research</p>
   </div>
  </div>}
