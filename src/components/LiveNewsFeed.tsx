        ...
        {loading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {Array.from({ length: Math.min(maxArticles, 6) }).map((_, i) => (
              <div key={i} className="rounded-xl border border-gray-200/40 dark:border-white/[0.06] bg-white dark:bg-[#201C16] p-5 animate-pulse">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {articles.map((article, i) => (
              <a
                ...
                className="rounded-xl border border-gray-200/40 dark:border-white/[0.06] bg-white dark:bg-[#201C16] p-5 hover:shadow-lg hover:border-gray-300 dark:hover:border-white/[0.12] transition-all duration-300 flex flex-col group"
              >
                <div className="flex items-center gap-2 text-[11px] text-gray-400 dark:text-gray-500 font-sans mb-3">
                  <span className="font-medium text-gray-500 dark:text-gray-400">{article.source}</span>
                  <span>&middot;</span>
                  <span>{formatPubDate(article.pubDate)}</span>
                </div>
                <h3 className="font-serif text-sm md:text-base text-gray-900 dark:text-[#F0EBE2] leading-snug mb-3 flex-1 group-hover:text-moss dark:group-hover:text-sage transition-colors">
                  {article.title}
                </h3>
                {article.description && (
                  <p className="text-xs text-gray-400 dark:text-gray-500 font-sans leading-relaxed line-clamp-2">
                    {article.description}
                  </p>
                )}
              </a>
