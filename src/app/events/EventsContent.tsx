...
export default function EventsContent() {
  ...
  return (
    <>
          ...
          <div className="border border-gray-200 rounded-xl overflow-hidden">
            ...
            <div className="grid grid-cols-7 border-b border-gray-200">
              {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((d) => (
                <div
                  key={d}
                  className="px-2 py-3 text-center text-xs font-sans uppercase tracking-wider text-gray-400"
                >
                  {d}
                </div>
              ))}
            ...
            <div className="grid grid-cols-7">
              ...
              {Array.from({ length: getDaysInMonth(calendarYear, calendarMonth) }).map((_, i) => {
                ...
                return (
                  ...
                  >
                    <span
                      className={`text-sm font-sans inline-flex items-center justify-center w-7 h-7 rounded-full ${
                        isToday
                    ...
                    {dayEvents.length > 0 && (
                      <div className="flex flex-wrap gap-1 mt-1">
                        {dayEvents.slice(0, 3).map((ev) => (
                          <span
                            key={ev.id}
                            className={`w-2 h-2 rounded-full ${categoryColors[ev.category].dot}`}
                            title={ev.title}
                          />
                        ))}
                      </div>
      ...
      {/* Event Cards Listing */}
      {activeTab !== "calendar" && (
        <section ref={eventsRef} className="max-w-6xl mx-auto px-6 pb-20">
    ...
    >
      ...
      >
        <div className="px-6 pb-6 border-t border-gray-100 pt-4">
          ...
          <div className="mb-5">
            <p className="text-xs text-gray-400 uppercase tracking-wider font-sans mb-2">
              Key Highlights
            </p>
