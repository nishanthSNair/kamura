...
export default function QuizContent() {
 ...
 return (
 <section className="min-h-screen bg-white pt-24 pb-20 zen-pattern">
 ...
 const prevOption = questions[currentQ - 1].options.find(
 (_, i) => questions[currentQ - 1].options[i].score === answers[answers.length - 1]
 );
 if (prevOption) {
 const newVotes = { ...archetypeVotes };
 newVotes[prevOption.archetype] = Math.max(0, newVotes[prevOption.archetype] - 1);
 setArchetypeVotes(newVotes);
 }
 setCurrentQ(currentQ - 1);
 setSelectedOption(null);
 }}
 className="flex items-center gap-1 text-xs text-gray-400 hover:text-gray-600 font-sans uppercase tracking-wider transition-colors"
 >
 ...
 selectedOption === i
 ? "border-sage bg-sage/5 text-gray-900"
 : selectedOption !== null
 ? "border-gray-100 text-gray-400 cursor-not-allowed"
 : "border-gray-200 text-gray-700 hover:border-sage/50 hover:bg-gray-50 cursor-pointer"
 }`}
 >
 ...
 <circle cx="60" cy="60" r="52" fill="none" className="stroke-gray-200" strokeWidth="8" />
 <circle
 cx="60"
 cy="60"
 r="52"
 ...
 className="flex items-center justify-between p-4 rounded-xl border border-gray-200 bg-white hover:border-sage/30 hover:shadow-sm transition-all group"
 >
 <div>
